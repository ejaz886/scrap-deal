...
<div>
  <div class="dashboard_table">
    <form id="update_form" action="<?php echo base_url()?>old_bill/add_new_item_qty" method="post"> 
    <table>
    <tr>
      <td colspan="2">
        <div><b>Bill No :  <?php if(!empty($_GET["Sale_Id"])){echo $_GET['Sale_Id'];}?></b></div>
    ...
    <tr>
      <td colspan="2" >
        <input type="text" id="barcode_scan" autofocus class="form-control form-control-sm" placeholder="Enter Barcode No">
      </td>
      <td colspan="2">
      ...
      <td colspan="2" align="center" style="width:90px">
...
<tr>
      ...
      <td align="center"><?php echo $si->Item_Code.$si->Item_Id?></td>
      <td align="left"><?php echo $si->Item_Category_Name?></td>
      <td align="left"><?php echo $si->Item_Discription?></td>
      <td align="center"><?php echo $si->Item_Unit?></td>
      <td align="center"><?php echo $si->Item_Rate?></td>
      <td align="center"><?php echo $si->Item_Qty?></td>
      <td align="center"><?php echo $si->Item_Discount?></td>
      <td align="center"> <?php echo $si->Item_Tax?></td>
    ...
    <tr style="font-weight: bold;">
      <td colspan="9" align="right">Item Discount</td>
    ...
    <tr style="font-weight: bold;">
      <td colspan="9" align="right">Bill Discount</td>
      <td align="right"><?php $grand = (($total+$item_tax_total)-$item_discount_total);
      $temp_x_dis =  (($grand * $sale_head->Sale_Overall_Discount)/100);
      echo $temp_x_dis;
    ?></td>
      <td>
        <input type="text" id="Sale_Overall_Discount" value="<?php echo $sale_head->Sale_Overall_Discount?>" style="width: 50px;">
      </td>
    </tr> 
    <tr style="font-weight: bold;">
    ...
    <a class="btn btn-xs btn-success" href="<?php echo base_url()?>old_bill/print_old_bill?Sale_Id=<?php if(!empty($_GET["Sale_Id"])){echo $_GET['Sale_Id'];}?>&final_amount=<?php echo ($grand - $temp_x_dis);?>"  target="_blank"><i class="fa fa-print"></i> Print</a></td>
    <td style="text-align:center">
...
<script>
  $("#top_button").hide();
  $("#top_heading").html("Old Bill Items");

  $("#barcode_scan").on("change",function(){
     $.ajax({
            type:"post",
            url:"<?php echo base_url()?>old_bill/add_by_barcode",
  ...
  $("#Sale_Overall_Discount").on("change",function(){
     $.ajax({
            type:"post",
            url:"<?php echo base_url()?>old_bill/update_overall_discount",
...
function delete_items(a)
{
  ...
  $.ajax({
            type:"post",
            url:"<?php echo base_url()?>old_bill/delete_item_sale",
            data:{"Sale_Item_Id":a},
            dataType:"text",
            success:function(resultData)
            {
  ...
  $("#item_category_id").on("change",function(){
     ...
     $.ajax({
            type:"post",
            url:"<?php echo base_url()?>old_bill/get_item",
               ...
               if(json_d.response)
                     {
                        ...
                        setTimeout(function(){
                         window.open("<?php echo base_url()?>old_bill","_self");
                     },1000);
                    }
                    else{
                        Toast.fire({
...
function save_bill()
{
  ...
  $.ajax({
            type:"post",
            url:"<?php echo base_url()?>old_bill/save_bill_final",
            data:{"Sale_Id":$("#Sale_Id").val()},
            dataType:"text",
            success:function(resultData)
            {
