...
class New_bill extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data["sale_head"] = $this->db->get("sale_head")->result();
        $data["page"] = "new_bill_page";
    ...
    public function set_bill_temp()
    {
        ...
        {
            $this->db->insert("sale_head",$this->input->post());
       ...
       public function add_new_item()
    {
        ...
        $data["sale_head"] = $this->db->get("sale_head")->row();


        $this->db->where("Sale_Id",$this->input->get("Sale_Id"));
        $data["sale_item"] = $this->db->get("sale_item")->result();


        $data["item_category"] = $this->db->get("item_category")->result();
        $data["page"] = "new_bill_item_page";
        $this->load->view("main_page",$data);
    }
    ...
    public function add_by_barcode()
    {
        if(!empty($this->input->post()))
        {
            ...
            $this->db->insert("sale_item",array(
    ...
    public function delete_item_sale()
    {
        ...
        $result = $this->db->delete("sale_item");
        $response = array("response"=>true,"msg"=>'Successfully Deleted...');
        }
        else{
            $response = array("response"=>false,"msg"=>'Empty Value....');
    ...
    public function delete_sale_head()
    {
        ...
        {
        $this->db->where("Sale_Id",$this->input->post("Sale_Id"));
        $result = $this->db->delete("sale_head");
     ...
     public function save_bill_final()
    {
        ...
        $this->db->where("Sale_Id",$this->input->post("Sale_Id"));
        $result = $this->db->get("sale_head")->row();
        ...
        $this->db->where("Sale_Id",$this->input->post("Sale_Id"));
        $result = $this->db->get("sale_item")->result();
        ...
        { 
            ...
            $this->db->where("Item_Id",$tt->Item_Id);
            $this->db->set('Item_Stock', "Item_Stock - ".$tt->Item_Qty, FALSE);
            $this->db->update("item");
        ...
        $response = array("response"=>true,"msg"=>'Successfully Created...',"Sale_Id"=>$last_id,"Grand_total"=>$this->input->post("grand_total"));
    }
        else{
            $response = array("response"=>false,"msg"=>'Empty Value....');
    ...
    public function get_item_price()
    {
        ...
        echo json_encode(array("Item_Rate"=>$result->Item_Rate));
        }
    } 
